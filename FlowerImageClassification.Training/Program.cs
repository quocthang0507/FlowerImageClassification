...
namespace FlowerImageClassification.Training
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			SetPaths();
			string output = Path.Combine(consoleOutputPath, $"inceptionv3_{DateTime.Now.ToString("d_M_y h_m_s")}.txt");
		...
		static void SetPaths()
		{
			...
			outputMlNetModelFilePath = Path.Combine(assetsPath, "Outputs", "imageClassifier_inceptionv3.zip");
